  ...
  private init(): void {
    ...
    const fragmentShader = this.compileShader(gl, gl.FRAGMENT_SHADER, `
      ...
      void main() {
        ...
        vec4 color = vec4(0.0);
        float f = 2.0 + fbm(p + vec2(time * 0.7, 0.0)) * 0.5;
        ...
        for (float i = 0.0; i < 35.0; i++) {
          ...
          vec4 aurora = vec4(
            0.35 + 0.25 * sin(i * 0.2 + time * 0.4),
            0.45 + 0.35 * cos(i * 0.3 + time * 0.5),
            0.72 + 0.28 * sin(i * 0.4 + time * 0.3),
            1.0
          );
        ...
        gl_FragColor = vec4(color.rgb * 1.45, 0.9);
      }
    `);
