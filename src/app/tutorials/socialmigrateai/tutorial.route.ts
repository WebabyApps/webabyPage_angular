...
const DATA: Record<string, TutorialContent> = {
  'socialmigrateai': {
    title: 'Social Migrate AI- Tutorial',
    intro: 'Move Your posts to new Profile.',
    blocks: [
      { type: 'image', src: 'cd ..', alt: 'Gameplay', caption: 'Gameplay example' },
...
const i18nResolver = () => firstValueFrom(
  forkJoin([
    ...
    inject(TranslocoService).selectTranslation('socialmigrateai') // ⬅️ scope = folder
  ])
);
...
export default [
  {
    path: '',
    data: { slug: 'socialmigrateai' }, // <- TutorialPage reads this when no :slug param
